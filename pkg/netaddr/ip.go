...
func IPAdd(ip net.IP, offset uint64) net.IP {
	if IsIPv4(ip) {
		a := uint64(ipToI32(ip[len(ip)-4:]))
		return i32ToIP(uint32(a + offset))
	}
...
func IPMod(ip net.IP, d uint64) uint64 {
	if IsIPv4(ip) {
		return uint64(ipToI32(ip[len(ip)-4:])) % d
	}
	b := uint64(d)
	hi := ipToU64(ip[:net.IPv6len/2])
	lo := ipToU64(ip[net.IPv6len/2:])
	return uint64(((hi%b)*((0-b)%b) + lo%b) % b)
}
